...
- **弹幕悬浮窗**（需 Chrome 116+ 或 Edge）— 点击工具栏画中画按钮，将弹幕列表弹出为独立小窗口，**始终置顶于所有应用之上**；弹幕内容（含表情图片）实时同步；点击用户名可直接在悬浮窗内弹出完整操作面板（禁言/标签/历史），无需切回浏览器；主题随主窗口深色/浅色模式自动同步；再次点击按钮或关闭小窗口即可退出

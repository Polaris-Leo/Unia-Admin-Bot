...
- **禁言日志** — 完整记录谁在何时对哪位用户执行了何种禁言操作，支持筛选和一键解禁；禁言时会回查 B 站禁言记录 ID，手动解禁后同步标记日志状态；1h/12h 等定时禁言到期后由后端定时检查，若 B 站仍未解除则自动解禁；「本场」禁言会在收到直播结束事件后延迟检查并自动处理，日志页展示「系统已自动解禁 / B站已解除 / 人工解除禁言 / 自动检查失败」等状态

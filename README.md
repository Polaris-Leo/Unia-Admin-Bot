...
- **实时弹幕控制台** — 弹幕/礼物/SC 分列展示，加载时自动预填当前场次最近 100 条弹幕；常驻筛选栏（用户名/UID/内容关键词实时高亮过滤）；弹幕内嵌表情按 B 站官方标准分两档渲染（标准颜文字 1 行高 / 订阅大表情 2 行高）
- **历史记录页** — 独立页面（`/history`）：左侧筛选栏（场次下拉、日期范围、用户名、UID、弹幕内容），右侧三列布局展示当场弹幕/SC/礼物；选定场次时各筛选条件在前端实时过滤，不选场次时跨全部场次搜索（最多 500 条）

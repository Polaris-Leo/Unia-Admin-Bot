...
- **禁言日志** — 完整记录谁在何时对哪位用户执行了何种禁言操作，支持筛选和一键解禁；禁言时会回查 B 站禁言记录 ID 用于精确匹配，解禁优先按用户 UID 调用官方接口；1h/12h 等定时禁言到期后由后端定时检查，若 B 站仍未解除则自动解禁；「本场」禁言会在收到直播结束事件后延迟检查并自动处理，日志页展示「系统已自动解禁 / B站已解除 / 人工解除禁言 / 自动检查失败」等状态
...
后端房管操作对接 B 站直播官方 Web 接口，并参考 NcatBot 对 `bilibili_api.live.LiveRoom` 的封装行为：

- 禁言：`POST https://api.live.bilibili.com/xlive/web-ucenter/v1/banned/AddSilentUser`
- 解禁：`POST https://api.live.bilibili.com/xlive/web-ucenter/v1/banned/DelSilentUser`
- 禁言参数使用 `room_id`、`tuid`、`hour`；当 `hour=0`（本场禁言）时额外传 `type=2`，其他时长传 `type=1`
- 解禁优先使用 `room_id + tuid`，不依赖 B 站禁言记录 ID；禁言记录 ID 仍会在禁言后回查并写入日志，用于自动检查时精确匹配
